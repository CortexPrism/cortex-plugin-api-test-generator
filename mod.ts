...
const apitest_generate: Tool = {
  definition: {
    ...
    params: [
      { name: "spec", type: "string", description: "URL or file path to OpenAPI spec", required: true },
      { name: "framework", type: "string", description: "Test framework to generate for", required: false, enum: ["jest", "vitest", "pytest", "go_test", "supertest"], default: "vitest" },
      { name: "output_dir", type: "string", description: "Directory to write generated tests", required: true },
    ],
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (!specArg || typeof specArg !== "string") {
        return { toolName: "apitest_generate", success: false, output: "", error: "spec must be a non-empty string", durationMs: Date.now() - start };
      }
      if (!output_dir || typeof output_dir !== "string") {
        return { toolName: "apitest_generate", success: false, output: "", error: "output_dir must be a non-empty string", durationMs: Date.now() - start };
      }
      const framework = typeof args.framework === "string" ? args.framework : (config.defaultFramework || "vitest");
      ...
      const extMap: Record<string, string> = { jest: "test.ts", vitest: "test.ts", pytest: "test.py", go_test: "_test.go", supertest: "test.ts" };
      const ext = extMap[framework] || "test.ts";
      const fwId = framework === "go_test" ? "go_test" : framework === "pytest" ? "pytest" : "vitest";
      ...
      if (framework === "pytest") {
        ...
        code = `import { describe, it, expect } from "${framework === "vitest" ? "vitest" : "@jest/globals"}";\n\nconst BASE_URL = "http://localhost:3000";\n\n`;
        for (const [path, methods] of Object.entries(paths)) {
          for (const [method] of Object.entries(methods)) {
      ...
      try { Deno.mkdirSync(output_dir, { recursive: true }); } catch { /* dir exists */ }
      const filePath = `${output_dir}/${filename}`;
      Deno.writeTextFileSync(filePath, code);

      return { toolName: "apitest_generate", success: true, output: `Generated ${Object.keys(paths).length} endpoint tests for framework "${framework}" at ${filePath}`, durationMs: Date.now() - start };
    } catch (error) {
      return { toolName: "apitest_generate", success: false, output: "", error: `Failed: ${error instanceof Error ? error.message : String(error)}`, durationMs: Date.now() - start };
    }
  },
};
...
const apitest_run: Tool = {
  definition: {
    ...
    params: [
      { name: "test_dir", type: "string", description: "Directory containing generated tests", required: true },
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (!test_dir || typeof test_dir !== "string") {
        return { toolName: "apitest_run", success: false, output: "", error: "test_dir must be a non-empty string", durationMs: Date.now() - start };
      }
      const framework = typeof args.framework === "string" ? args.framework : (config.defaultFramework || "vitest");
      ...
      return { toolName: "apitest_run", success: false, output: "", error: `Failed: ${error instanceof Error ? error.message : String(error)}`, durationMs: Date.now() - start };
    }
  },
};
...
const apitest_validate_spec: Tool = {
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      const specArg = args.spec;
      if (!specArg || typeof specArg !== "string") {
        return { toolName: "apitest_validate_spec", success: false, output: "", error: "spec must be a non-empty string", durationMs: Date.now() - start };
      }
      const raw = await fetchSpec(specArg);
      ...
      if (errors.length > 0) {
        return { toolName: "apitest_validate_spec", success: false, output: "", error: `Validation errors: ${errors.join("; ")}`, durationMs: Date.now() - start };
      }
      const pathsCount = Object.keys(specObj.paths).length;
      return { toolName: "apitest_validate_spec", success: true, output: `Valid OpenAPI ${specObj.openapi} spec with ${pathsCount} paths`, durationMs: Date.now() - start };
    } catch (error) {
      return { toolName: "apitest_validate_spec", success: false, output: "", error: `Failed: ${error instanceof Error ? error.message : String(error)}`, durationMs: Date.now() - start };
    }
  },
};
...
const apitest_list_endpoints: Tool = {
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      const specArg = args.spec;
      if (!specArg || typeof specArg !== "string") {
        return { toolName: "apitest_list_endpoints", success: false, output: "", error: "spec must be a non-empty string", durationMs: Date.now() - start };
      }
      const raw = await fetchSpec(specArg);
      const specObj = parseSpec(raw);
      const paths = (specObj.paths as Record<string, Record<string, Record<string, unknown>>>) || {};
      ...
      return { toolName: "apitest_list_endpoints", success: true, output: lines.join("\n"), durationMs: Date.now() - start };
    } catch (error) {
      return { toolName: "apitest_list_endpoints", success: false, output: "", error: `Failed: ${error instanceof Error ? error.message : String(error)}`, durationMs: Date.now() - start };
    }
  },
};
...
const apitest_mock_server: Tool = {
  definition: {
    ...
    params: [
      { name: "spec", type: "string", description: "URL or file path to OpenAPI spec", required: true },
      { name: "port", type: "number", description: "Port to run the mock server on", required: false, default: 3000 },
    ],
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (!specArg || typeof specArg !== "string") {
        return { toolName: "apitest_mock_server", success: false, output: "", error: "spec must be a non-empty string", durationMs: Date.now() - start };
      }
      const raw = await fetchSpec(specArg);
      const specObj = parseSpec(raw);
      ...
      serverCode += `  if (!match) return new Response(JSON.stringify({ error: "Not found" }), { status: 404, headers: { "Content-Type": "application/json" } });\n`;
      serverCode += `  const methodEntry = match[req.method.toLowerCase()];\n`;
      serverCode += `  if (!methodEntry) return new Response(JSON.stringify({ error: "Method not allowed" }), { status: 405, headers: { "Content-Type": "application/json" } });\n`;
      serverCode += `  const schema = (methodEntry.responses?.["200"] || methodEntry.responses?.["201"])?.content?.["application/json"]?.schema;\n`;
      ...
      serverCode += `      else if (prop.type === "number" || prop.type === "integer") sample[k] = 0;\n`;
      serverCode += `      else if (prop.type === "boolean") sample[k] = true;\n`;
      serverCode += `      else sample[k] = null;\n`;
      serverCode += `    }\n`;
      serverCode += `  }\n`;
      serverCode += `  return new Response(JSON.stringify(sample), { status: 200, headers: { "Content-Type": "application/json" } });\n`;
      serverCode += `};\n`;
      ...
      return { toolName: "apitest_mock_server", success: true, output: `Mock server started on http://localhost:${port}`, durationMs: Date.now() - start };
    } catch (error) {
      return { toolName: "apitest_mock_server", success: false, output: "", error: `Failed: ${error instanceof Error ? error.message : String(error)}`, durationMs: Date.now() - start };
    }
  },
};
